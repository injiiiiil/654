...
class Driver:
    ...
    def getNewStream(self, device_idx, priority):
        return self.run_on_exeuctor(device_idx, "getNewStream", priority)

    @register(registry)
    def queryStream(self, stream):
        return self.run_on_exeuctor(
    ...
    def synchronizeStream(self, stream):
        self.run_on_exeuctor(stream.device_index, "synchronizeStream", stream.stream_id)
