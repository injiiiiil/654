...
class TestLinalg(TestCase):
    ...
    @skipCUDAIfNotRocm
    @dtypes(torch.half)
    def test_matmul_hipblaslt_tunableop(self, device, dtype):
        # Tune a matrix multiply but ONLY with hipBLASLt
        # To avoid a false positive, we tune a GEMM for which
        # ROCBLAS has the more optimal solution on gfx90a
        ...
        try:
            ...
            # Deactivate ROCBLAS (default) in order to generate only hipBLASLt tunings
            os.environ["PYTORCH_TUNABLEOP_ROCBLAS_ENABLED"] = "0"
            ...
            # This stores total number of cummulative results
            gemm_result = torch.cuda.tunable.get_results()

            total_num_results = len(gemm_result)
            ...
            gemm_signature = 'nn_4_4_4'
            gemm_library = 'Gemm_Hipblaslt'
            gemm_last_entry_signature = gemm_result[-1][1]
            gemm_last_entry_library = gemm_result[-1][2]
            self.assertTrue(gemm_last_entry_signature.startswith(gemm_signature))
            self.assertTrue(gemm_last_entry_library.startswith(gemm_library))
    ...
    @skipCUDAIfNotRocm
    @dtypes(torch.half)
    def test_matmul_rocblas_tunableop(self, device, dtype):
        # Tune a matrix multiply but only with ROCBLAS
        # To avoid a false positive, we tune a GEMM for which
        # ROCBLAS has the more optimal solution on gfx90a
        ...
        try:
            ...
            # Deactivate hipBLASLt (default) in order to generate only ROCBLAS tunings
